...
void QPlaceSearchSuggestionReplyMapbox::replyFinished()
{
    ...
    for (int i = 0; i < features.count(); ++i) {
        const QJsonObject feature = features.at(i).toObject();
        suggestions.append(feature.value(QStringLiteral("text")).toString());
    }
...
void QPlaceSearchSuggestionReplyMapbox::networkError(QNetworkReply::NetworkError error)
{
    Q_UNUSED(error)
    QNetworkReply *reply = static_cast<QNetworkReply *>(sender());
    reply->deleteLater();
