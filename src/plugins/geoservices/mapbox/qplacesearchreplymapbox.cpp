...
QPlaceResult parsePlaceResult(const QJsonObject &response, const QString &attribution)
{
    ...
    // XXX: type, place_type, relevance
    // XXX: properties.short_code, properties.landmark, properties.wikidata
    ...
    // guaranteed. Your implementation should check for the presence of these
    ...
    if (response.value(QStringLiteral("properties")).isObject()) {
        ...
        if (!categoryString.isEmpty()) {
            ...
            foreach (const QString &categoryId, categoryString.split(QStringLiteral(", "), QString::SkipEmptyParts)) {
...
void QPlaceSearchReplyMapbox::replyFinished()
{
    ...
    QList<QPlaceCategory> categories = request().categories();

    QList<QPlaceSearchResult> results;
    for (int i = 0; i < features.count(); ++i) {
        QPlaceResult placeResult = parsePlaceResult(features.at(i).toObject(), attribution);
...
void QPlaceSearchReplyMapbox::networkError(QNetworkReply::NetworkError error)
{
    Q_UNUSED(error)
    QNetworkReply *reply = static_cast<QNetworkReply *>(sender());
    reply->deleteLater();
